...
if ARGV.size > 0
    # puts "Help menu:"
    puts "pmdb is the interactive database behind phantommist".colorize(:green)
    puts "pmdb does not have any cmd line flags and is entirely interactive.... (for now at least)".colorize(:red)
    puts ""
...
while true 
    ...
    if ans
        ...
        when "sprayed"
            ...
            ######################### export features ############################
        when "export sprayed"
            puts "Exporting SPRAYED to exported_pmdb_sprayed.csv ... "
            File.delete("./exported_pmdb_sprayed.csv") if File.exists? "./exported_pmdb_sprayed.csv"
            file = File.new("./exported_pmdb_sprayed.csv", "w")
            ...
            puts ""
        when "export valid"
            puts "Exporting VALID to exported_pmdb_valid.csv ... "
            File.delete("./exported_pmdb_valid.csv") if File.exists? "./exported_pmdb_valid.csv"
            file = File.new("./exported_pmdb_valid.csv", "w")
            file.puts "Username,Password,Time"
            begin 
                db.query "select username.username, invalid_usernames.date_time, invalid_usernames.spraytype
                from username, invalid_usernames
            ...
            puts ""
        when "export invalid"
            puts "Exporting INVALID to exported_pmdb_invalid.csv ... "
            File.delete("./exported_pmdb_invalid.csv") if File.exists? "./exported_pmdb_invalid.csv"
            file = File.new("./exported_pmdb_invalid.csv", "w")
        ...
        when "export val-users"
            puts "Exporting usernames minus invalid usernames to exported_pmdb_val-users.csv ... "
            File.delete("./exported_pmdb_val-users.csv") if File.exists? "./exported_pmdb_val-users.csv"
            file = File.new("./exported_pmdb_val-users.csv", "w")
        ...
        when "export passwords"
            puts "Exporting VALID to exported_pmdb_passwords.csv ... "
            File.delete("./exported_pmdb_passwords.csv") if File.exists? "./exported_pmdb_passwords.csv"
            file = File.new("./exported_pmdb_passwords.csv", "w")
        ...
        when "export usernames"
            puts "Exporting VALID to exported_pmdb_usernames.csv ... "
            File.delete("./exported_pmdb_usernames.csv") if File.exists? "./exported_pmdb_usernames.csv"
            file = File.new("./exported_pmdb_usernames.csv", "w")
        ...
        when "help"
            puts "pmdb: the backend database util for phantommist!"
            puts "VERSION: #{VERSION}"
            puts "#{" " * 2}Commands:"
